(ns furniture.sim
  ...
  flammability-compliance screening -> repair completion (escalate/
  approve/commit) -> item return (escalate/approve/commit), then shows
  HARD-hold scenarios: a jurisdiction with no spec-basis, a parts-cost
  mismatch (verified first), a failed post-repair safety test (screened
  directly via `:safety/screen`), an unconfirmed flammability-
  compliance status on a reupholstered ticket (screened directly via
  `:flammability/screen` -- never via an actuation op against an
  unscreened ticket -- see this actor's own governor ns docstring / the
  lesson `parksafety`'s ADR-2607071922 Decision 5, `eldercare`'s,
  ...
  `socialresearch`'s, `bizassoc`'s, `vocational`'s and `training`'s
  ADR-0001s already recorded), a double completion, and a double
  return."
...
(defn -main [& _]
  (let [db (store/seed-db)
        actor (op/build db)]
    (println "== ticket/intake ticket-1 (JPN, clean; no upholstery work) ==")
    ...
    (println "== flammability/screen ticket-1 (no upholstery work; escalates -- human approves) ==")
    (println (exec-op actor "t3b" {:op :flammability/screen :subject "ticket-1"} operator))
    ...
    (println "== flammability/screen ticket-5 (reupholstered, compliance unconfirmed -> HARD hold, never reaches a human) ==")
    (println (exec-op actor "t10" {:op :flammability/screen :subject "ticket-5"} operator))
