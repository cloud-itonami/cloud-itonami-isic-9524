(ns furniture.registry
  "Pure-function repair-completion + item-return record construction --
  an append-only furniture-repair-shop book-of-record draft, closely
  modeled on `cloud-itonami-isic-9522`'s `applianceshop.registry`.
  ...
  honest, non-fabricating discipline `furniture.facts` uses.

  `parts-cost-matches-claim?` is an HONEST, LITERAL reuse of
  `repairshop.registry`'s/`commrepair.registry`'s/`applianceshop.
  registry`'s own EXACT-MATCH independent-recompute shape, the SAME
  real-world concern (a claimed parts cost must equal parts-quantity x
  parts-unit-price) reapplied to furniture-repair tickets (hardware,
  fabric yardage, foam/filling material) rather than consumer-
  electronics, communication-equipment or household-appliance repair
  tickets -- not claimed as new.
...
(defn parts-cost-matches-claim?
  ...
  is an honest, literal reuse of `applianceshop.registry`'s own check,
  not a new concept."
...
(defn register-repair-completion
  ...
  the RECORD a shop would keep. `furniture.governor` independently
...
(defn register-item-return
  ...
  shop's own legal act of returning a real furniture item to the
  customer. Pure function -- does not touch any real repair-shop-
  management system; it builds the RECORD a shop would keep.
  `furniture.governor` independently re-verifies the ticket's own
  post-repair safety-test status and flammability-compliance status
  (when applicable), and blocks a double-return of the same ticket,
