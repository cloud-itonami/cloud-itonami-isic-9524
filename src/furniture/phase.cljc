(ns furniture.phase
  "Phase 0->3 staged rollout -- the furniture-and-home-furnishings-
  repair analog of `cloud-itonami-isic-9522`'s `applianceshop.phase`.
    ...
    Phase 2  assisted-assess  -- adds jurisdiction assessment + safety
                                 screening + flammability-compliance
  ...
  owner's call. `furniture.governor`'s `:actuation/complete-repair`/
  `:actuation/return-item` high-stakes gate enforces the same invariant
  independently -- two layers, not one, agree on this. `:safety/
  screen`/`:flammability/screen` are likewise never auto-eligible, at
  any phase -- the same posture every sibling's screening op has. Like
  `repairshop.phase`'s/`commrepair.phase`'s/`applianceshop.phase`'s
  (and every prior sibling's) phase 3 `:auto` set, this domain has
  only ONE member (`:ticket/intake`) -- no separate no-capital-risk
  'file' lifecycle distinct from the ticket itself.")

(def read-ops  #{})
(def write-ops #{:ticket/intake :jurisdiction/assess :safety/screen
                 :flammability/screen :repair/complete :item/return})
...
(def phases
  ...
  {0 {:label "read-only"       :writes #{}                                                                                    :auto #{}}
   ...
   2 {:label "assisted-assess" :writes #{:ticket/intake :jurisdiction/assess :safety/screen :flammability/screen}             :auto #{}}
