(ns furniture.governor-contract-test
  "The governor contract as executable tests -- the furniture-and-
  home-furnishings-repair analog of `cloud-itonami-isic-9521`'s
  ...
  test`), and structurally closest to `cloud-itonami-isic-9522`'s
  `applianceshop.governor-contract-test`. The single invariant under
  test:
...
(deftest parts-cost-mismatch-is-held
  (testing "a claimed parts cost that doesn't equal quantity x unit-price -> HOLD (honest reuse of repairshop/9521's and applianceshop/9522's own check)"
    (let [[db actor] (fresh)
...
(deftest safety-test-not-passed-is-held-and-unoverridable
  (testing "a failed post-repair safety test on a ticket -> HOLD, and never reaches request-approval (honest reuse of repairshop/9521's and applianceshop/9522's own check)"
    (let [[db actor] (fresh)
...
(deftest flammability-compliance-unconfirmed-is-held-and-unoverridable
  (testing "an unconfirmed flammability-compliance status on a reupholstered ticket -> HOLD, and never reaches request-approval -- exercised via :flammability/screen DIRECTLY, not via an actuation op against an unscreened ticket -- the genuinely NEW check this vertical adds, the 67th unconditional-evaluation-discipline grounding overall and the FOURTH conditional variant (see this actor's governor ns docstring / parksafety's ADR-2607071922 Decision 5 / eldercare's, museum's, conservation's, salon's, entertainment's, casework's, hospital's, facility's, school's, association's, leasing's, behavioral's, secondary's, card's, water's, telecom's, aerospace's, recovery's, consulting's, union's, congregation's, fab's, energy's, care's, navigator's, learning's, banking's, advertising's, polling's, research's, design's, nursing's, sports's, alliedhealth's, laundry's, holdco's, photo's, personalservice's, edsupport's, headoffice's, residential's, cultural's, reserve's, proserv's, sportsevent's, recreation's, sportsclub's, partyops's, memberorg's, commrepair's, applianceshop's, socialresearch's, bizassoc's, vocational's and training's ADR-0001s)"
    (let [[db actor] (fresh)
          res (exec-op actor "t7" {:op :flammability/screen :subject "ticket-5"} operator)]
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:flammability-compliance-unconfirmed} (-> (store/ledger db) first :basis)))
      (is (nil? (store/flammability-screening-of db "ticket-5")) "no clearance written"))))

(deftest flammability-screen-is-a-noop-when-no-upholstery-work
  (testing "the flammability-compliance check is CONDITIONAL: a repair that does not involve upholstery work has no flammability-compliance requirement at all"
    (let [[_db actor] (fresh)
          res (exec-op actor "t7b" {:op :flammability/screen :subject "ticket-1"} operator)]
