(ns furniture.store-contract-test
...
(deftest read-parity
  (doseq [[label s] (backends)]
    (testing label
      ...
      (is (false? (:involves-upholstery-work? (store/ticket s "ticket-1"))))
      (is (= 120.0 (:claimed-parts-cost (store/ticket s "ticket-3"))))
      (is (false? (:safety-test-passed? (store/ticket s "ticket-4"))))
      (is (true? (:involves-upholstery-work? (store/ticket s "ticket-5"))))
      (is (false? (:flammability-compliance-confirmed? (store/ticket s "ticket-5"))))
      ...
      (is (nil? (store/flammability-screening-of s "ticket-1")))
...
(deftest write-and-ledger-parity
  (doseq [[label s] (backends)]
    (testing label
      ...
      (testing "assessment / safety-screening / flammability-screening payloads commit and read back"
        ...
        (store/commit-record! s {:effect :flammability-screening/set :path ["ticket-1"]
                                 :payload {:ticket-id "ticket-1" :verdict :not-applicable}})
        (is (= {:ticket-id "ticket-1" :verdict :not-applicable} (store/flammability-screening-of s "ticket-1"))))
...
(deftest datomic-empty-store-is-usable
  (let [s (store/datomic-store)]
    ...
    (is (zero? (store/next-completion-sequence s "JPN")))
    (is (zero? (store/next-return-sequence s "JPN")))
    (store/with-tickets s {"x" {:id "x" :customer "n" :item "d" :item-type :chair
